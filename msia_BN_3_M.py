...
def pool_upsamping_3_M(input_feature, level, training, name):
    if level == 1:
        with tf.variable_scope(name):
            pu_conv = slim.conv2d(input_feature, input_feature.get_shape()[-1], [3, 3], 1, padding='SAME',
                                  scope='pu_conv')
        ...
        with tf.variable_scope(name):
            ...
            pu_conv = tf.layers.batch_normalization(pu_conv, training=training)
            pu_conv = tf.nn.relu(pu_conv)
            conv_up = slim.conv2d_transpose(pu_conv, input_feature.get_shape()[-1], [2, 2], 2, padding='SAME',
                                            scope='conv_up')
    elif level == 4:
        with tf.variable_scope(name):
            ...
            pu_conv = tf.layers.batch_normalization(pu_conv, training=training)
            pu_conv = tf.nn.relu(pu_conv)
            conv_up_1 = slim.conv2d_transpose(pu_conv, input_feature.get_shape()[-1], [2, 2], 2, padding='SAME',
                                              scope='conv_up_1')
            conv_up = slim.conv2d_transpose(conv_up_1, input_feature.get_shape()[-1], [2, 2], 2, padding='SAME',
                                            scope='conv_up')
...
def Multi_Scale_Module_3_M(input_feature, training, name):
    ...
    multi_scale_feature = slim.conv2d(res, input_feature.shape[3], [1, 1], 1, padding='SAME',
                                      scope=name + 'multi_scale_feature')

...
parser.add_argument('--checkpoint_dir', dest='checkpoint_dir', default="./experiment/exp2/checkpoint")
...
for idx in range(len(eval_low_data)):
    ...
    ratio3 = np.mean((decom_r_low) / (restoration_r + 0.0001))
    ratio4 = np.mean(1 / (decom_i_low + 0.0001))

    if ratio3 > 0.7:
        i_low_data_ratio = np.ones([h, w]) * (ratio4 - ratio3)
    else:
        i_low_data_ratio = np.ones([h, w]) * (ratio4 - ratio3 * 2)
    # print('ratio:', i_low_data_ratio)
    ...
    save_images(os.path.join(sample_dir, '%s_xiaomai_v2.png' % (name)), fusion)
